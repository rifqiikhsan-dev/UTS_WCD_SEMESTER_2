...
const ListPokemon = ({ view, sortBy, keyword }) => {
    ...
    useEffect(() => {
        ...
        const storedPokemonData = getFromLocalStorage("pokemonData");

        // If not, save the dummy data to localStorage
        if (!storedPokemonData) {
            saveToLocalStorage("pokemonData", dummyData);
        }
    }, []);

    // Get data from localStorage or use dummy data as fallback
    const pokemonData = getFromLocalStorage("pokemonData") || dummyData;

    const filteredData = pokemonData.filter((pokemon) => pokemon.name.toLowerCase().includes(keyword.toLowerCase()));
    ...
    const handlePokemonClick = (pokemon) => {
        ...
        saveToLocalStorage("selectedPokemon", pokemon);
