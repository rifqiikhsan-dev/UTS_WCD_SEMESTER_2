...
const PokemonDetail = () => {
    ...
    useEffect(() => {
        // Try to get the pokemon from state first
        if (state?.pokemon) {
        ...
        const selectedPokemon = getFromLocalStorage("selectedPokemon");

        if (selectedPokemon && selectedPokemon.id.replace("#", "") === id) {
            setPokemon(selectedPokemon);
            return;
        }

        // If not found in localStorage as selected, fetch from all pokemon
        const allPokemon = getFromLocalStorage("pokemonData");
        if (allPokemon) {
            const foundPokemon = allPokemon.find((p) => p.id.replace("#", "") === id);
            if (foundPokemon) {
        ...
        // If still not found, we don't have the data
            ...
            <div className="bg-[#252A3E] min-h-screen w-screen flex flex-col">
                ...
                <div className="text-center mt-10 text-white">
                    ...
                    <div className="mt-4">
                        <button
                            onClick={() => navigate("/")}
                            className="bg-[#3D4466] py-2 px-4 rounded-lg text-white hover:bg-[#4D5580] transition-colors duration-200"
                        >
                            Kembali ke Daftar
                        </button>
                    </div>
